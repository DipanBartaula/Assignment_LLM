"""
visualize_attention_activations.py - Visualize attention mechanisms in Nepali text classification

This module provides tools to visualize attention weights and hidden state activations
from BART and CANINE models during Nepali text classification. It supports visualizing:
- Single attention head heatmaps
- All attention heads from a specific layer
- Token-level attention scores
- Cross-layer attention flow patterns

Visualizations use Nepali words from the original text for maximum interpretability.
"""
...
def main(args):
    ...
    tokenizer_name = tokenizer_map.get(args.model_name, 'facebook/mbart-large-cc25')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Visualize attention mechanisms in trained Nepali text classifiers'
    )
    parser.add_argument(
        '--model_name',
        type=str,
        required=True,
        choices=['bart', 'canine'],
        help='Model architecture: BART or CANINE'
    )
    parser.add_argument(
        '--text',
        type=str,
