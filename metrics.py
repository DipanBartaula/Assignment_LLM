"""
metrics.py - Comprehensive evaluation metrics for Nepali text classification

This module computes a wide range of classification metrics on the test set including:
...
All metrics are computed on the test set from the dataloader.
"""
import os
import numpy as np
import torch
...
def calculate_metrics(
    ...
    Computes a complete suite of evaluation metrics including:
    - Standard metrics: accuracy, precision, recall, F1-score (macro and weighted)
    - Advanced metrics: Matthews Correlation Coefficient, Cohen's Kappa
    - Probabilistic metrics: ROC-AUC (one-vs-rest for multiclass)
    - Per-class metrics: precision, recall, F1 for each category
    
    Args:
        y_true: True labels
        y_pred: Predicted labels
        y_logits: Raw model logits before softmax
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Evaluate model on test set using comprehensive metrics'
    )
    parser.add_argument(
        '--model_name',
        type=str,
